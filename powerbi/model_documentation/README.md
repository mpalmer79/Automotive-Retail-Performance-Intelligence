...
| Target-attainment measures | `warehouse.fact_sales_target` is Deferred. There is nothing to attain against, and a target invented for the purpose would be a fictional operating goal presented as a benchmark. |
| Measure tables for the four Deferred groups | F&I, Customer Retention, Service to Sales and Target Attainment exist in [03-measure-groups.md §9](03-measure-groups.md) and nowhere else: no table, no empty table, no placeholder measure, no hidden stub. |
