...
|---|---|---|
| F&I measures | **The semantic-model binding, not the data.** `warehouse.fact_finance_product_sale` and `warehouse.fact_finance_product_adjustment` were built by `DASH.6`, and twenty-two `KPI-FNI-*` definitions are computable from `reporting` today. **No TMDL table, relationship or DAX measure was added**, so this group is blocked only by the absent binding — the same position Target Attainment is in. Until it is bound, `KPI-GRS-002` remains a single number **in Power BI** with no product detail behind it; in SQL it is now explained to the cent by `RECON-FI-001`. |
