...
| [ADR-0014](ADR-0014-gate-2-external-manual-validation-dependency.md) | Gate 2 as an External Manual Validation Dependency | Accepted | 2026-08-08 | Reclassifies Gate 2 real-engine validation as an **external manual dependency** rather than a work item, so that it stops blocking and stops being restated in increments it does not touch. `DASH.9`–`DASH.13` proceed unless an increment directly requires a real Microsoft engine result. Fixes the five situations in which the status must be mentioned and the single line that suffices otherwise, while keeping the prohibition on overstating absolute — the repository state stays **PENDING** on both paths and ADR-0008 is untouched. Authorizes a **SIMULATED SEMANTIC-MODEL VALIDATION** layer as a development proxy: two independent implementations of the governed definitions, compared; and it may never be labelled Power BI, Desktop or Fabric validated, or Gate 2 passed. |
| [ADR-0016](ADR-0016-social-card-as-an-illustrative-raster.md) | The Social Card as an Illustrative Raster | Accepted | 2026-08-14 | Makes `public/brand/social-preview.png` the sole canonical Open Graph card at 1200 × 630, committed at the path it is served from, and retires both the `brand/social-preview.svg` master and the rendered `public/social-preview.png` output along with the `npm run assets` target that produced it. **Withdraws the `DASH.13` rule that every figure on the social card is governed output**, because a raster has no text to reconcile and the supplied asset would not pass: it labels 92 as `ROSI`, prints `Total Sales $3,499` for what is governed gross per retail unit, and shows 64% inventory health and a 20.5% close rate against a governed 40.4% aged inventory and 1.5% lead-to-sale conversion. Records the withdrawal openly rather than deleting the test quietly, and moves the honesty rules a raster still admits onto the card's alternative text — which must declare the rendering illustrative, may quote no currency amount, percentage or unit count, may claim no business result, and stays under the ADR-0012 fairness freeze. Leaves every reconciliation rule outside the social card untouched: no KPI definition, denominator, selector or business calculation changes. |
...
`scripts/check_simulation_labels.py` enforces ADR-0014 by failing the build when a tracked file describes the
simulated semantic-model validation as a real-engine one, or when the simulated artifact records an engine
state the evidence files do not support.
