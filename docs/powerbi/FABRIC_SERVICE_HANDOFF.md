...
Two routes. Both end in the same place; the first is automated.

### Route A — Railway (automated; preferred)

[`deployment/railway/README.md`](../../deployment/railway/README.md) automates the whole of
section 4. Bootstrap creates the database with a persistent volume, TLS and a TCP proxy; a
one-time `arpi-database-setup` job then runs the ordered SQL sequence, creates the role
boundary, loads the `development` profile, runs `scripts/verify_cloud_database.py`, and
**proves as the real login** that `arpi_fabric` is refused on `raw`, `staging`, `warehouse`
and `audit`.

At the end of a provisioning run the job prints the non-secret handoff coordinates:

| Field             | Where it comes from                                                       |
| ----------------- | ------------------------------------------------------------------------- |
| host              | `RAILWAY_TCP_PROXY_DOMAIN` — Fabric connects from Microsoft's network, so the **TCP proxy** is the route in, not the private domain |
...
**The password is not printed, anywhere, by anything.** It is generated by Railway, lives
only in Railway's variable store as `ARPI_FABRIC_PASSWORD` on the `arpi-database-setup`
service, and is deliberately absent from GitHub Actions output, workflow summaries, PR
comments, repository files, deployment logs, JSON artefacts and chat messages.

**Reveal it in the Railway dashboard, in a browser, and paste it straight into Fabric's
credential store in §6.** Do not route it through a terminal, a file, a ticket or a
message. Once Fabric holds its own copy, consider sealing the Railway variable — see
[`deployment/railway/README.md`](../../deployment/railway/README.md) section 8.

### Route B — any other managed provider (manual)
...
profile, and verifies that `arpi_reporter` can read `reporting` and nothing else. Sections 4
onward of that document are provider-neutral.
...
You will finish holding:

- a host name and port,
- a database name,
- a login role granted `arpi_reporter`, and its password.

**Status: neither route has been executed.** No cloud database exists. Route A needs one
Railway API token and approval of a cost; Route B needs an account somewhere.
