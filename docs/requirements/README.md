...
| [PHASE_2_BACKLOG.md](PHASE_2_BACKLOG.md) | The Phase 2 work backlog, organised into delivery increments `P2.1` through `P2.4` — semantic model, dashboard pages, findings and the Gate 2 review, portfolio packaging — with acceptance criteria, tests, dependencies and explicit non-goals for every item. Carries the Gate 2 checklist and the Power BI Desktop validation gate. |
| [DASHBOARD_PROGRAM.md](DASHBOARD_PROGRAM.md) | The master program for the ARPI Dealer Operations Command Center — the governed web operating console authorized by [ADR-0013](../architecture-decisions/ADR-0013-governed-web-operating-console.md): purpose, boundaries, personas, route and data architecture, data-model expansion, risks, release criteria, and explicit non-goals. Its specification set lives in [`../dashboard/`](../dashboard/). |
| [DASHBOARD_BACKLOG.md](DASHBOARD_BACKLOG.md) | The dashboard program's delivery increments `DASH.0` through `DASH.13` (plus recorded `DASH.O-*` optional items), with permanent item identifiers `DASH.<n>-NN`, acceptance criteria, required tests, data-grain impact, and completion evidence for every item. The `DASH` family is deliberately disjoint from lifecycle phases and the `P1.x`/`P2.x` increments. |
