...
`DASH.13-02` sets the budgets, from measurements taken once `UX.2` is complete. Setting one here
would fix a number for nine routes while `UX.2D` is still changing how they behave together.
...
## 9.15 `UX.2C` — 16,196 px for 31.4 kB, all of it markup
...
the shell, and the routes' own figures contribute nothing to it. CSS moved 48.0 → 48.1 kB across
the whole console.
...
Nothing was optimised away to reach these numbers and nothing was hidden behind a client fetch.
A route that draws more marks carries more markup; 17 kB of gzipped HTML for a route that lost
4,823 px and half its visible prose is the trade the increment makes, and it is recorded rather
than rounded off.
