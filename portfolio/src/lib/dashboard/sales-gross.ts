...
import { buildBridge, type BridgeState } from './change-drivers'

// Re-exported so the page and its tests keep importing the bridge's types from the module
// they already import its view model from. `DASH.12` moved the implementation, not the API.
export type { BridgeComponent, BridgeState } from './change-drivers'
import { grossChangeBridgeRows } from './change-drivers-data'
import { salesGrossTrendRows } from './sales-gross-data'
