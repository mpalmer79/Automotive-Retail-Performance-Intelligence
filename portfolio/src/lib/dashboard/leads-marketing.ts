...
/**
 * Why a figure is not a number, in the project's four-way vocabulary.
 *
 * `DATA_CONTRACT.md`'s absence rule, applied here rather than collapsed into one dash:
 *
 *   not-applicable    structurally meaningless. Cost per lead for a walk-in.
 ...
 * walk-in, and only one of them is true.
 */
export type AbsenceKind = 'not-applicable' | 'no-data' | 'not-at-this-grain'
...
/**
 * An order statistic over the exported population, weighted by each bin's lead count.
 *
 * THIS IS THE ONLY WAY A MEDIAN MAY BE FORMED HERE. A median does not decompose: the median
 * of a month is not the average of its daily medians, not the average of its store medians,
 * and not a weighted blend of either — all three are different numbers and all three are
 * wrong. `lead-response` publishes medians at store x source x day and they cannot be
 * combined, which is why this route reads the DISTRIBUTION instead.
 *
 ...
 * fastest end and improve the median, which is the defect
 * `leads-marketing.test.ts` seeds explicitly.
 */
