...
import { ROUTES, SYNTHETIC_DATA_STATEMENT } from '@/lib/site'
...
export default async function DealJacketPage({
  ...
  return (
    <Canvas>
      <PageHeader
        eyebrow="Deal Jacket"
        title={`${jacket.identity.saleId}: ${jacket.vehicle.display}`}
        crumbLabel={jacket.identity.saleId}
        lede={`A sanitized record of one finalized transaction at ${jacket.identity.storeName}, delivered ${jacket.identity.deliveryDate}. Every figure is the deal's own exported value, and the two arithmetic identities are recomputed on this page from the components shown.`}
        dashboardNav
        ...
        meta={
          <>
            ...
            <Badge tone={anyFailure ? 'pending' : 'verified'}>
              {anyFailure
