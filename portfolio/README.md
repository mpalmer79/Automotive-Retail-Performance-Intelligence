...
Thirteen indexable routes, plus one that is not.
...
The header carries **seven** content destinations plus GitHub, not thirteen:
...
`/governance` too. `Dealerships` points at `/dealerships` and is the current item
on all three store pages. Each group renders a shared sub-navigation -
`PlatformNav` and `GroupNav` - that links its members with `aria-current`, so
every route stays directly addressable, indexable and one click away.
