...
class ImportSummary:
    ...
    Attributes:
        ...
        calendar_dates_added: Calendar rows this import had to add to
            ``warehouse.dim_date`` so the conformed dimension covered the capture.
    ...
    def summary(self) -> str:
        ...
        lines = [
            ...
            f"fact rows total    : {self.fact_rows_total}",
            f"calendar dates added: {self.calendar_dates_added}",
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "calendar_dates_added": self.calendar_dates_added,
            "reconciliations": [
                {
...
def _extend_calendar(cursor: Any, captures: Sequence[date]) -> int:
    """Ensure ``warehouse.dim_date`` covers every capture date, and return how many it added.

    WHY AN IMPORT MAY LEGITIMATELY EXTEND THE CALENDAR
    --------------------------------------------------
    ``warehouse.dim_date`` is a CONFORMED dimension: every date any fact references has to
    be in it, and the listing fact's foreign key to it is what lets a listing report share
    a calendar with every other ARPI report.

    A capture date is a property of when somebody looked at a public website. It has no
    reason to fall inside the reporting window a synthetic operational dataset happened to
    be generated for -- the committed Granite Chevrolet capture is 2026-08-02, and no
    profile's window contains it. Without this step the fact load's inner join would drop
    every row, the reconciliation would fail, and the import would be refused for a reason
    that is not the operator's fault and that no option could fix.

    So the import extends the calendar rather than failing on it. The rows are produced by
    ARPI's own calendar generator, which is a pure function of the date and the holiday
    rules, so a date added here is byte-identical to the same date generated by a pipeline
    run whose window did cover it. Nothing is overwritten: ``ON CONFLICT DO NOTHING``
    means an existing calendar row is authoritative and this can only ever add.

    This is the ONLY object outside the listing lane that an import writes to, and it
    writes no measure -- only the calendar rows the fact it is about to load requires.
    """
    wanted = sorted(set(captures))
    if not wanted:
        return 0
    frame = build_calendar_rows(min(wanted), max(wanted))
    frame = frame[frame["full_date"].dt.date.isin(wanted)]
    if frame.empty:  # pragma: no cover - unreachable while wanted is non-empty
        return 0

    columns = list(frame.columns)
    placeholders = ", ".join(["%s"] * len(columns))
    statement = (
        f"INSERT INTO warehouse.dim_date ({', '.join(columns)}) "
        f"VALUES ({placeholders}) ON CONFLICT (date_key) DO NOTHING"
    )
    added = 0
    ...
    if added:
        _LOGGER.info(
            "Extended warehouse.dim_date with %d calendar date(s) the capture requires.", added
        )
    return added
...
def import_listing_workbook(  # noqa: PLR0913 - every argument is an operator decision
    ...
    *,
    ...
    with connection.cursor() as cursor:
        ...
        # The conformed calendar has to cover the capture before the fact that
        # references it is loaded. See _extend_calendar for why an import may add to it.
        calendar_dates_added = _extend_calendar(cursor, [record.captured_at for record in records])
        ...
        return ImportSummary(
            ...
            calendar_dates_added=calendar_dates_added,
