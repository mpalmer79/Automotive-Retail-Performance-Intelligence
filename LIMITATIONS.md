...
| **A sale does not imply a modelled showroom visit** | `warehouse.fact_lead` enforces that an appointment implies contact and a show implies an appointment, but NOT that a sale implies a show: **175 of 400 sold leads in the development profile never showed**. `appointment_shown_leads - sold_leads` is therefore not the count of leads that showed without buying and goes negative on some rows. It is also why `RECON-FUNNEL-CHAIN` is informational: the chain of stage rates only approximates `KPI-FUN-006`, and the difference is the walk-in-later-matched path rather than an error. `reporting.vw_lead_stage_loss` partitions by furthest stage reached for this reason. |
| **Response-time KPIs are blind to leads nobody answered** | `KPI-FUN-007` and `KPI-FUN-008` both exclude leads with no first response, so a store that ignores half its leads can report an excellent median. The unanswered count and the coverage rate are published beside both measures wherever either is shown; they are not a footnote. |
| **No benchmark exists for any funnel or marketing measure** | ARPI holds no benchmark data for response time, contact rate, appointment-set rate, show rate, cost per lead, cost per sale or gross return, so it publishes no target for any of them and no figure on the leads and marketing surface is coloured good or bad. Comparison is across stores, sources and periods only. |
| **The scheduled-date and show-date bases never separate in the generated data** | `KPI-FUN-004` is on the scheduled date and `KPI-FUN-005` on the show date, and the distinction is real in the model. It is **unexercised**: `0` of `1,025` shown appointments have a show date different from their scheduled date, because the generator never produces a customer who arrives on another day. Any reasoning about late arrivals is reasoning about a case this dataset does not contain. |
...
| Forward migrations | 6 | Ordered, immutable once released, recorded in `audit.schema_migration`. |
...
| Dashboard program SQL files (ADR-0013) | 43 | Implemented. |
...
| Dashboard program reporting views | 15 | Implemented, and **not** part of the reporting-view baseline the semantic model binds to. |
