...
| 147 | `06_indexes/01_phase1_indexes.sql` | Creates the Phase 1 justified secondary indexes |
| 148 | `06_indexes/02_inventory_listing_indexes.sql` | **Listing lane.** Secondary indexes for the listing fact and its dimension |
| 149 | `06_indexes/03_fi_indexes.sql` | **Dashboard program lane.** Secondary indexes for the two F&I facts and for `fact_vehicle_sale.lender_key` |
| 150 | `06_indexes/04_accounting_indexes.sql` | **Accounting control lane.** Secondary indexes for the two accounting facts: the account-led grouping the reconciliation performs, and the vehicle partition `KPI-ACC-011` needs |
| 151 | `07_security/00_roles.sql` | Creates `arpi_admin`, `arpi_loader`, `arpi_reporter` (NOLOGIN) |
| 152 | `07_security/01_grants.sql` | Moves ownership to `arpi_admin`; applies the grant model; asserts it object by object |
| 153 | `08_validation/00_validation_helpers.sql` | Result-shape template view + `audit.fn_record_validation_result` |
| 154 | `08_validation/01_dim_date_checks.sql` | Creates `audit.vw_dq_dim_date` (`DQ-DATE-001..005`) |
| 155 | `08_validation/02_dim_dealership_checks.sql` | Creates `audit.vw_dq_dim_dealership` (`DQ-DLR-001..005`) |
| 156 | `08_validation/03_referential_checks.sql` | Creates `audit.vw_dq_referential` (`DQ-REF-001..005`) |
| 157 | `08_validation/04_audit_checks.sql` | Creates `audit.vw_dq_audit` (`DQ-AUD-001..005`), `audit.vw_dq_all`, `audit.fn_record_all_dq_checks` |
| 158 | `08_validation/05_reconciliation_helpers.sql` | Creates `audit.vw_recon_result_template`, the uniform reconciliation result shape |
| 159 | `08_validation/06_recon_ingestion.sql` | Creates `audit.vw_recon_ingestion` — the five facts' staging-to-warehouse counts, and snapshot continuity |
| 160 | `08_validation/07_recon_gross.sql` | Creates `audit.vw_recon_gross` — `RECON-GROSS-001`, `RECON-GROSS-002`, `RECON-UNITS-001`, `RECON-REPORT-SALES` |
| 161 | `08_validation/08_recon_funnel.sql` | Creates `audit.vw_recon_funnel` — `RECON-LEAD-001`, duplicates, funnel bounds, sold path, funnel chain |
| 162 | `08_validation/09_recon_marketing.sql` | Creates `audit.vw_recon_marketing` — spend, attributed leads, sales and gross, and the cost-attributability rule |
| 163 | `08_validation/10_recon_reporting.sql` | Creates `audit.vw_recon_reporting` — every reporting view reconciled to the fact it projects |
| 164 | `08_validation/11_recon_target.sql` | **Dashboard program lane.** Creates `audit.vw_recon_target` — the target chain, the declared grain, the department partition of the store gross plan, the store and month totals through reporting, and the fan-out guard on `reporting.vw_target_attainment`. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 165 | `08_validation/12_recon_inventory_listing.sql` | **Listing lane.** Creates `audit.vw_recon_inventory_listing` — `RECON-LISTING-001..010`. Deliberately **not** unioned into `audit.vw_recon_all`, which is the pipeline's per-run set with an asserted per-run count; this lane runs on a workbook cadence |
| 166 | `08_validation/13_recon_fi.sql` | **Dashboard program lane.** Creates `audit.vw_recon_fi` — `RECON-FI-001` and the rest of the F&I family: the two chains, both grains, the product price identity, that `total = front + back` is exactly as true after `DASH.6` as before it, the reserve-and-lender structure rule, the eligibility subset, the adjustment cap and sequence, as-of net gross, and the fan-out guard on all four F&I views. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 167 | `08_validation/14_recon_inventory_units.sql` | **Dashboard program lane.** Creates `audit.vw_recon_inventory_units` — `RECON-INV-UNIT-RATIO`, which re-proves that `reporting.vw_inventory_units` and `reporting.vw_inventory_snapshots` state the same `price_to_market_ratio` on every shared row (the unit view reads the fact directly for its window functions, so it REPEATS that expression rather than selecting it, and NULL is compared as a value so an absent estimate must give an absent ratio on both sides), and `RECON-INV-UNIT-GRAIN`, which proves the reportable-date narrowing neither duplicates nor drops a snapshot. Numbered 14 because `DASH.8` vacated that slot, so no existing file moves. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 168 | `08_validation/15_recon_accounting.sql` | **Accounting control lane.** Creates `audit.vw_recon_accounting` — `RECON-ACC-BOOK-IDENTITY` and the rest of the family: the two chains, both grains, the negative-component guard, that pack is still outside the front-gross identity, that floorplan is outside the book identity while being materially non-zero, the population at matched dates, the per-account totals and the mapping behind them, the well-formedness of every GL comparison, and the fan-out guards on two views. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 169 | `08_validation/16_recon_all.sql` | Creates `audit.vw_recon_all` and `audit.fn_record_all_reconciliations`. Renumbered from 11 by `DASH.5`, from 13 by `DASH.6` and from 14 by `DASH.8`, so `audit.vw_recon_target`, `audit.vw_recon_fi`, `audit.vw_recon_inventory_units` (added by `DASH.9` into the number `DASH.8` vacated) and `audit.vw_recon_accounting` all exist before this file unions them |
| 170–175 | `09_migrations/0000_migration_history.sql` … `0005_add_inventory_market_price_estimate.sql` | The migration ledger and the five recorded migrations. Each is idempotent and records itself. `0003` is the second to contain DDL rather than only assertions: `CREATE TABLE IF NOT EXISTS` on a live table is a no-op, so `fact_vehicle_sale.finance_reserve_gross` and `lender_key` would never appear on a deployed database without it. `0004` creates nothing, because `DASH.8` changes no existing table — it asserts the whole accounting lane arrived, including the book-value identity and both declared grains. `0005` contains DDL for the same reason `0003` does: `DASH.9` adds `fact_vehicle_inventory_snapshot.market_price_estimate`, and without it `reporting.vw_inventory_snapshots` would fail to create on a deployed database, surfacing a missing column as a broken reporting layer |
| 176 | `07_security/01_grants.sql` **(again)** | Privilege-normalisation pass over the objects created in the validation and migration steps |

The sequence is **176 files** in total; the table groups consecutive files of one kind
