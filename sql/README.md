...
| 76 | `03_dimensions/26_employee_governed_functions.sql` | **Dashboard program lane (`DASH.11`).** Creates `warehouse.fn_employee_role_family` — THE map deciding which employee-performance surface a job role's measured activity belongs to. Every branch was derived by auditing which job roles actually appear in each role-playing foreign key, not assumed from the titles: Sales Manager and General Manager are Desk Management because both are credited on real deliveries in `desk_manager_key`, and Service Advisor returns NULL because `fact_service_visit` is Deferred and no fact credits one. NULL rather than a default on an unknown role, so an unmapped title fails `RECON-EMP-ROLE-COMPAT` instead of silently acquiring another family's denominators |
| 77 | `04_facts/00_fact_vehicle_sale.sql` | Creates `warehouse.fact_vehicle_sale` |
| 78 | `04_facts/01_fact_vehicle_inventory_snapshot.sql` | Creates `warehouse.fact_vehicle_inventory_snapshot` |
| 79 | `04_facts/02_fact_lead.sql` | Creates `warehouse.fact_lead` |
| 80 | `04_facts/03_fact_appointment.sql` | Creates `warehouse.fact_appointment` |
| 81 | `04_facts/04_fact_marketing_spend.sql` | Creates `warehouse.fact_marketing_spend` |
| 82 | `04_facts/05_fact_vehicle_listing_snapshot.sql` | **Listing lane.** Creates `warehouse.fact_vehicle_listing_snapshot` — a **sixth** fact that is not one of the five MVP facts and is not read by the semantic model |
| 83 | `04_facts/06_fact_sales_target.sql` | **Dashboard program lane.** Creates `warehouse.fact_sales_target` — a **seventh** fact table, and the second that is not one of the five MVP facts and is not read by the semantic model. The monthly operating PLAN: store, department or employee scope × targeted KPI × month, with the grain enforced over five NOT NULL columns |
| 84 | `04_facts/07_fact_finance_product_sale.sql` | **Dashboard program lane (`DASH.6`).** Creates `warehouse.fact_finance_product_sale` — an **eighth** fact table. One row per product contract on a finalized deal, grain `(sale_key, finance_product_key)`, with `original_product_gross = price − cost` as a `CHECK` |
| 85 | `04_facts/08_fact_finance_product_adjustment.sql` | **Dashboard program lane (`DASH.6`).** Creates `warehouse.fact_finance_product_adjustment` — a **ninth** fact table. One row per post-sale EVENT on its own business date; the contract it acts on is never rewritten |
| 86–87 | `04_facts/09_fact_inventory_accounting_snapshot.sql` … `10_fact_gl_control_balance.sql` | **Accounting control lane (`DASH.8`).** Creates `warehouse.fact_inventory_accounting_snapshot` — a **tenth** fact, carrying the book-value identity as a `CHECK` so a violating line is unloadable — and `warehouse.fact_gl_control_balance`, an **eleventh** fact that deliberately carries **no** constraint requiring agreement with the subledger, because a variance is valid data |
| 88–92 | `04_facts/10_fact_vehicle_sale_load.sql` … `14_fact_marketing_spend_load.sql` | **Runtime.** The five MVP fact load scripts, one per fact above, each guarded by `ON CONFLICT ... DO UPDATE` |
| 93 | `04_facts/15_fact_vehicle_listing_snapshot_load.sql` | **Listing lane. Workbook cadence.** Insert-only: `ON CONFLICT DO NOTHING` with **no UPDATE path at all**, because a capture records what somebody observed at a moment that has passed and cannot be recomputed |
| 94 | `04_facts/16_fact_sales_target_load.sql` | **Dashboard program lane. Runtime.** Idempotent load of the operating plan, conflict-targeted on the declared grain. A later planning revision replaces the row rather than adding one: a plan is a current statement, not an event log |
| 95–96 | `04_facts/17_fact_finance_product_sale_load.sql` … `18_fact_finance_product_adjustment_load.sql` | **Dashboard program lane. Runtime.** 17 sorts after 10 so the sale fact is populated before a contract resolves `sale_key` against it, and 18 after 17 for the same reason: an adjustment resolves the contract it acts on |
| 97–98 | `04_facts/19_fact_inventory_accounting_snapshot_load.sql` … `20_fact_gl_control_balance_load.sql` | **Accounting control lane. Runtime.** Both loads CALCULATE NOTHING: `current_book_value` is carried through so the `CHECK` can fail on a generator defect rather than agreeing with a total the load just computed, and `net_balance` is loaded exactly as it arrives so the reconciliation surface can show a variance rather than only ever agreement |
| 99 | `05_reporting/00_reporting_scope.sql` | Documents which reporting views exist and which are deliberately absent |
| 100–103 | `05_reporting/01_vw_calendar.sql` … `04_vw_data_quality_summary.sql` | The four Phase 0 views: `vw_calendar`, `vw_dealership`, `vw_pipeline_run_summary`, `vw_data_quality_summary` |
| 104–109 | `05_reporting/05_vw_vehicle_model.sql` … `10_vw_marketing_campaign.sql` | The six remaining **dimension** views: `vw_vehicle_model`, `vw_vehicle`, `vw_employee`, `vw_customer`, `vw_lead_source`, `vw_marketing_campaign` |
| 110–114 | `05_reporting/11_vw_vehicle_sales.sql` … `15_vw_marketing_spend.sql` | The five **fact** views, each preserving its fact's grain exactly: `vw_vehicle_sales`, `vw_inventory_snapshots`, `vw_leads`, `vw_appointments`, `vw_marketing_spend` |
| 115–127 | `05_reporting/20_vw_sales_summary.sql` … `32_vw_reconciliation_status.sql` | The thirteen governed **analytical** views: sales, gross, inventory health, inventory aging, days to sale, inventory turn, days supply, lead funnel, appointment funnel, lead response, marketing performance, data-quality trend, reconciliation status |
| 128–133 | `05_reporting/33_vw_vehicle_listing_current.sql` … `38_vw_vehicle_listing_change.sql` | **Listing lane.** The six listing views: `vw_vehicle_listing_current`, `vw_vehicle_listing_summary`, `vw_vehicle_listing_model_mix`, `vw_vehicle_listing_price_completeness`, `vw_vehicle_listing_observation_span`, `vw_vehicle_listing_change`. Counted **apart from the twenty-eight**; see §1.1 |
| 134–138 | `05_reporting/40_vw_sales_gross_trend.sql` … `44_vw_target_attainment.sql` | **Dashboard program lane.** The three `DASH.3` views, one from `DASH.4` and one from `DASH.5`: `vw_sales_gross_trend` (store × sale date, volume and gross with their condition components as additive columns), `vw_gross_change_bridge` (store × month pair × component, published as exact numerators over a shared denominator so the reconciliation needs no division), `vw_deal_explorer` (one row per finalized transaction, public-safe and compact, for the index), `vw_deal_jacket` (the same grain, presentation-complete: the cost components behind the front gross, the trade context, the finance amounts, the people and the lead's paper trail, for the record view), `vw_target_attainment` (store × target scope × targeted KPI × month: the operating plan beside the month-to-date actual, with the selling-day arithmetic `KPI-TGT-001..010` are computed from, published as numerators and denominators so no consumer can average a percentage). Counted **apart from the twenty-eight**; see §1.1 |
| 139–142 | `05_reporting/45_vw_deal_product_detail.sql` … `48_vw_fi_adjustment_summary.sql` | **Dashboard program lane (`DASH.6`).** The four F&I views: `vw_deal_product_detail` (one row per contract, with deal-date and as-of gross side by side), `vw_fi_summary` (store × sale date × finance manager — the category is deliberately NOT in the grain, because the view carries reserve and retail units), `vw_fi_product_penetration` (adds the category, and carries NO reserve and NO retail-unit column for the same reason), `vw_fi_adjustment_summary` (the only F&I view on the ADJUSTMENT date). Counted **apart from the twenty-eight**; see §1.1 |
| 143–145 | `05_reporting/49_vw_inventory_accounting.sql` … `51_vw_accounting_exceptions.sql` | **Accounting control lane (`DASH.8`).** The three accounting views: `vw_inventory_accounting` (the stock schedule at the fact's own grain, with every book component published separately and floorplan principal carried as liability context), `vw_inventory_gl_reconciliation` (the matched-date FULL JOIN of both sides, with the signed variance and an explicit comparison state — a missing side is NULL, never zero), `vw_accounting_exceptions` (one row per exception over a closed code vocabulary). Counted **apart from the twenty-eight**; see §1.1 |
| 146 | `05_reporting/52_vw_inventory_units.sql` | **Dashboard program lane (`DASH.9`).** The operating console's unit-grain inventory surface: business identifiers and denormalised vehicle description, so no surrogate key crosses the browser boundary, plus snapshot-derived markdown activity and `price_to_market_ratio`. Same grain as `vw_inventory_snapshots`, different job — that view is the semantic-model fact view and publishes surrogate keys instead. Counted **apart from the twenty-eight**; see §1.1 |
| 147–149 | `05_reporting/53_vw_appointment_source_funnel.sql` … `55_vw_lead_response_distribution.sql` | **Dashboard program lane (`DASH.10`).** Three presentation-grain views over existing facts, for the leads and marketing route. `vw_appointment_source_funnel` carries KPI-FUN-004 and KPI-FUN-005 at lead-source and campaign grain, resolved through the one lead each appointment came from, so a source-filtered page can scope the appointment rates with the same filter it scopes the funnel above them; `vw_lead_stage_loss` partitions the lead cohort by furthest stage reached, as FILTERs over disjoint predicates rather than differences of sums, because `fact_lead` does not enforce that a sale implies a show; `vw_lead_response_distribution` publishes the first-response population as counted bins, which is the only shape a median can be recomputed from at an arbitrary filter scope. None adds a fact, a dimension or a KPI identifier. Counted **apart from the twenty-eight**; see §1.1 |
| 150–151 | `05_reporting/56_vw_employee_performance.sql` … `57_vw_employee_lead_source_response.sql` | **Dashboard program lane (`DASH.11`).** The two employee-performance views. `vw_employee_performance` publishes role-aware components at store × date × role family × employee **version** — the SCD Type 2 version the fact points at, never the current one, so history keeps its store and its title. Its sale columns are named per credit relationship (`sold_`, `desked_`, `financed_`) because one delivery is credited to three people and a shared unit column would triple every delivery for anything that summed it. `vw_employee_lead_source_response` carries the assigned-lead population beneath that grain, by source and by distinct first-response value, which is what makes lead-source mix and a TRUE median possible without either fanning the employee row out. Neither ranks anybody: both publish components, never rates |
| 152 | `06_indexes/00_indexes.sql` | Creates the Phase 0 justified secondary indexes |
| 153 | `06_indexes/01_phase1_indexes.sql` | Creates the Phase 1 justified secondary indexes |
| 154 | `06_indexes/02_inventory_listing_indexes.sql` | **Listing lane.** Secondary indexes for the listing fact and its dimension |
| 155 | `06_indexes/03_fi_indexes.sql` | **Dashboard program lane.** Secondary indexes for the two F&I facts and for `fact_vehicle_sale.lender_key` |
| 156 | `06_indexes/04_accounting_indexes.sql` | **Accounting control lane.** Secondary indexes for the two accounting facts: the account-led grouping the reconciliation performs, and the vehicle partition `KPI-ACC-011` needs |
| 157 | `07_security/00_roles.sql` | Creates `arpi_admin`, `arpi_loader`, `arpi_reporter` (NOLOGIN) |
| 158 | `07_security/01_grants.sql` | Moves ownership to `arpi_admin`; applies the grant model; asserts it object by object |
| 159 | `08_validation/00_validation_helpers.sql` | Result-shape template view + `audit.fn_record_validation_result` |
| 160 | `08_validation/01_dim_date_checks.sql` | Creates `audit.vw_dq_dim_date` (`DQ-DATE-001..005`) |
| 161 | `08_validation/02_dim_dealership_checks.sql` | Creates `audit.vw_dq_dim_dealership` (`DQ-DLR-001..005`) |
| 162 | `08_validation/03_referential_checks.sql` | Creates `audit.vw_dq_referential` (`DQ-REF-001..005`) |
| 163 | `08_validation/04_audit_checks.sql` | Creates `audit.vw_dq_audit` (`DQ-AUD-001..005`), `audit.vw_dq_all`, `audit.fn_record_all_dq_checks` |
| 164 | `08_validation/05_reconciliation_helpers.sql` | Creates `audit.vw_recon_result_template`, the uniform reconciliation result shape |
| 165 | `08_validation/06_recon_ingestion.sql` | Creates `audit.vw_recon_ingestion` — the five facts' staging-to-warehouse counts, and snapshot continuity |
| 166 | `08_validation/07_recon_gross.sql` | Creates `audit.vw_recon_gross` — `RECON-GROSS-001`, `RECON-GROSS-002`, `RECON-UNITS-001`, `RECON-REPORT-SALES` |
| 167 | `08_validation/08_recon_funnel.sql` | Creates `audit.vw_recon_funnel` — `RECON-LEAD-001`, duplicates, funnel bounds, sold path, funnel chain |
| 168 | `08_validation/09_recon_marketing.sql` | Creates `audit.vw_recon_marketing` — spend, attributed leads, sales and gross, and the cost-attributability rule |
| 169 | `08_validation/10_recon_reporting.sql` | Creates `audit.vw_recon_reporting` — every reporting view reconciled to the fact it projects |
| 170 | `08_validation/11_recon_target.sql` | **Dashboard program lane.** Creates `audit.vw_recon_target` — the target chain, the declared grain, the department partition of the store gross plan, the store and month totals through reporting, and the fan-out guard on `reporting.vw_target_attainment`. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 171 | `08_validation/12_recon_inventory_listing.sql` | **Listing lane.** Creates `audit.vw_recon_inventory_listing` — `RECON-LISTING-001..010`. Deliberately **not** unioned into `audit.vw_recon_all`, which is the pipeline's per-run set with an asserted per-run count; this lane runs on a workbook cadence |
| 172 | `08_validation/13_recon_fi.sql` | **Dashboard program lane.** Creates `audit.vw_recon_fi` — `RECON-FI-001` and the rest of the F&I family: the two chains, both grains, the product price identity, that `total = front + back` is exactly as true after `DASH.6` as before it, the reserve-and-lender structure rule, the eligibility subset, the adjustment cap and sequence, as-of net gross, and the fan-out guard on all four F&I views. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 173 | `08_validation/14_recon_inventory_units.sql` | **Dashboard program lane.** Creates `audit.vw_recon_inventory_units` — `RECON-INV-UNIT-RATIO`, which re-proves that `reporting.vw_inventory_units` and `reporting.vw_inventory_snapshots` state the same `price_to_market_ratio` on every shared row (the unit view reads the fact directly for its window functions, so it REPEATS that expression rather than selecting it, and NULL is compared as a value so an absent estimate must give an absent ratio on both sides), and `RECON-INV-UNIT-GRAIN`, which proves the reportable-date narrowing neither duplicates nor drops a snapshot. Numbered 14 because `DASH.8` vacated that slot, so no existing file moves. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 174 | `08_validation/15_recon_accounting.sql` | **Accounting control lane.** Creates `audit.vw_recon_accounting` — `RECON-ACC-BOOK-IDENTITY` and the rest of the family: the two chains, both grains, the negative-component guard, that pack is still outside the front-gross identity, that floorplan is outside the book identity while being materially non-zero, the population at matched dates, the per-account totals and the mapping behind them, the well-formedness of every GL comparison, and the fan-out guards on two views. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 175 | `08_validation/16_recon_leads_marketing.sql` | **Leads and marketing lane.** Creates `audit.vw_recon_leads_marketing` — `RECON-APPT-SOURCE-ROLLUP`, `RECON-LEAD-STAGE-PARTITION`, `RECON-LEAD-STAGE-GRAIN`, `RECON-LEAD-RESPONSE-DIST-ROLLUP` and `RECON-LEAD-RESPONSE-DIST-MEDIAN`: the three `DASH.10` presentation-grain views against the authorities they re-grain, component by component rather than by ratio, plus the median recomputed from the response bins against the median over the governed lead population. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 176 | `08_validation/17_recon_employee_performance.sql` | **Dashboard program lane (`DASH.11`).** Creates `audit.vw_recon_employee_performance` — the thirteen `RECON-EMP-*` rules: both declared grains, the role-family map against every role-playing key, historical SCD Type 2 attribution (the defect that moves August units to a December store while every total still balances), both sale credits against `reporting.vw_vehicle_sales` with no family filter, the finance credit against `reporting.vw_fi_summary` at full grain, every lead and appointment component on all three date bases, the activity credited to nobody, each published mix as a partition, the supporting view's rollup and its true per-employee median, and that the sample floor comes from `warehouse.fn_minimum_sample_floor()` and nowhere else. Unioned into `audit.vw_recon_all`, which is why it is numbered before it |
| 177 | `08_validation/18_recon_all.sql` | Creates `audit.vw_recon_all` and `audit.fn_record_all_reconciliations`. Renumbered from 11 by `DASH.5`, from 13 by `DASH.6`, from 14 by `DASH.8`, from 16 by `DASH.10` and from 17 by `DASH.11`, so `audit.vw_recon_target`, `audit.vw_recon_fi`, `audit.vw_recon_inventory_units` (added by `DASH.9` into the number `DASH.8` vacated), `audit.vw_recon_accounting`, `audit.vw_recon_leads_marketing` and `audit.vw_recon_employee_performance` all exist before this file unions them |
| 178–183 | `09_migrations/0000_migration_history.sql` … `0005_add_inventory_market_price_estimate.sql` | The migration ledger and the five recorded migrations. Each is idempotent and records itself. `0003` is the second to contain DDL rather than only assertions: `CREATE TABLE IF NOT EXISTS` on a live table is a no-op, so `fact_vehicle_sale.finance_reserve_gross` and `lender_key` would never appear on a deployed database without it. `0004` creates nothing, because `DASH.8` changes no existing table — it asserts the whole accounting lane arrived, including the book-value identity and both declared grains. `0005` contains DDL for the same reason `0003` does: `DASH.9` adds `fact_vehicle_inventory_snapshot.market_price_estimate`, and without it `reporting.vw_inventory_snapshots` would fail to create on a deployed database, surfacing a missing column as a broken reporting layer |
| 180 | `07_security/01_grants.sql` **(again)** | Privilege-normalisation pass over the objects created in the validation and migration steps |

The sequence is **184 files** in total; the table groups consecutive files of one kind
...
The sequence is 176 files, so pasting each one is not the readable option.
