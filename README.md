...
| F&I presentation surface | **Not started, by decision** | `DASH.6` delivered the SQL, generation, validation and reporting layers and **no console page, no itemized Deal Jacket, no F&I manager view and no browser dataset**. `DASH.7` owns the presentation surface, and a test asserts that no F&I view appears in the dashboard export contract |
...
| Reconciliation suite | Implemented | Ninety-six results recorded on every database run; every critical rule proven to fail against a corrupted fixture |
